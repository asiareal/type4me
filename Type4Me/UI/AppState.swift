...
struct ProcessingMode: Codable, Identifiable, Equatable, Hashable {
    ...
    你是划词问答助手。用户选中了一段文本，并通过语音提出了一个问题或指令。
    ...
    4. 如果用户要求总结、改写、解释、找问题或给建议，就按该意图直接处理选中文本。
    5. 不要编造选中文本之外的事实；不确定时明确说明。
    6. 除非用户只要求短答案或翻译，否则使用清晰的 Markdown 排版。段落之间保留空行，不要输出一个很长的连续段落。
    7. 每段最多 1-2 句话；需要列举时使用项目符号或编号列表。
